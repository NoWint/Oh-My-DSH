...
|-------|------|---------------------|
| ⭐ 168.5k | [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | Official core: **Everything is a Plugin.** Plugin-based agent harness powered by [Cordis](https://github.com/cordiverse/cordis). / 官方核心：**万物皆可插件。** 基于 Cordis 的插件化 agent 框架。 |
...
| Stars | Repo | Description / 描述 |
|-------|------|---------------------|
| ⭐ 15.45k | [anywhere-labs/deepseek-harness-desktop](https://github.com/anywhere-labs/deepseek-harness-desktop) | Modern Electron desktop for DeepSeek Harness — optimized for macOS & Windows with best out-of-box experience. / 现代化 Electron 桌面端，深度适配 macOS 与 Windows，最佳开箱即用体验。 |
...
| ⭐ 5 | [ninipa/dsh-desktop](https://github.com/ninipa/dsh-desktop) | A desktop shell for DeepSeek Harness (dsh). / DSH 桌面壳。 |
...
| ⭐ 1,141 | [dataelement/dsh-desktop](https://github.com/dataelement/dsh-desktop) | Desktop for DeepSeek Harness. / DeepSeek Harness 桌面客户端。 |
...
| ⭐ 0 | [kasparovabi/dsh-studio](https://github.com/kasparovabi/dsh-studio) | Unofficial native macOS (SwiftUI) client for dsh — sessions, live trajectory, tool approvals, models, goals, subagents. / 非官方 macOS 原生客户端（SwiftUI）：会话、实时轨迹、工具审批、模型切换、目标管理、子代理。 |
| ⭐ 1 | [WZZNNE/DSH-CyberWorkStation](https://github.com/WZZNNE/DSH-CyberWorkStation) | One-click DSH bundle: cyberpunk desktop launcher, SillyTavern-graded Control Deck, 8 production plugins, AI skin studio & markets. Vendored core rc.8. / DSH 赛博工作站整合包：赛博朋克桌面启动器、SillyTavern 级别控制面板、8 个生产插件、AI 皮肤工作室。 |
...
| Stars | Repo | Description / 描述 |
|-------|------|---------------------|
| ⭐ 2.1k | [ccch1mneyyy/dsh-TUI](https://github.com/ccch1mneyyy/dsh-TUI) | Claude Code-style fullscreen interactive terminal: pixel whale header, streaming thought display, double-Esc rollback, TPS gauge. npm one-click install. / Claude Code 风格全屏交互终端：像素鲸鱼顶栏、思考流式展开、双击 Esc 回滚、TPS 仪表，npm 一键安装。 |
...
| ⭐ 0 | [ipromise2021/dsh-omc-tui](https://github.com/ipromise2021/dsh-omc-tui) | DeepSeek Harness native full-featured terminal interaction interface — Claude Code-styled Terminal TUI & CLI. / DSH 原生全功能终端交互界面，Claude Code 风格 TTY & CLI。 |
...
| ⭐ 74 | [liangmianya/dsh-synapse](https://github.com/liangmianya/dsh-synapse) | A visual, non-linear conversation workspace plugin for DeepSeek Harness — a canvas-based session explorer and branching workspace. / DSH 可视化非线性会话工作区插件：画布式会话浏览器与分支工作空间。 |
...
| ⭐ 0 | [PerryLink/dsh-kit](https://github.com/PerryLink/dsh-kit) | One-command starter pack: install all 15 PerryLink DeepSeek Harness plugins. / 一键启动包：安装全部 15 个 PerryLink DSH 插件。 |
| ⭐ 1 | [PerryLink/dsh-data-quality](https://github.com/PerryLink/dsh-data-quality) | DeepSeek Harness plugin: deterministic data profiling, cleaning, and verification. / DSH 插件：确定性数据剖析、清洗与验证。 |
...
| ⭐ 6 | [v587d/dsh-opencode-go-usage](https://github.com/v587d/dsh-opencode-go-usage) | A DeepSeek Harness (dsh) bundle that shows OpenCode Go subscription usage in the Web GUI composer dock — same seat as built-in conversation stats. / DSH bundle：在 Web GUI 组合器面板显示 OpenCode Go 订阅用量，与内置对话统计行同位置。 |
...
| ⭐ 4 | [Pan17/dsh-wechat](https://github.com/Pan17/dsh-wechat) | DSH plugin bridging WeChat private messages — supports text, images, files, audio and video bidirectional transmission; targets native WeChat experience in DSH. / DSH 插件将微信私聊消息桥接到 DSH，支持文本、图片、文件、音视频双向传输，目标是在微信端还原 DSH 的原生体验。 |
...
| ⭐ 1 | [NattoCB/dsh-plugin-wechat-bridge](https://github.com/NattoCB/dsh-plugin-wechat-bridge) | WeChat (ilink bot) bridge plugin for DeepSeek Harness: one DSH agent session per peer per day. / DSH 微信(iLink bot)桥接插件：每个联系人每天一个 DSH Agent 会话。 |
...
| ⭐ 26 | [flymysql/dsh-remote](https://github.com/flymysql/dsh-remote) | Remote-work assistant for DeepSeek Harness: connect via SSH (key or password), pick workspace, and run agent sessions remotely. / DSH 远程工作助手：SSH 连接（密钥或密码）、选择工作区、远程运行 Agent 会话。 |
| ⭐ 12 | [Blank-not-black/dsh-Remote](https://github.com/Blank-not-black/dsh-Remote) | DSH Remote: pocket DSH console — sessions, approvals, questions, file transfer over LAN/Tailscale. Auto fastest-server selection, offline chat history. Token-authenticated, data only between your devices. / DSH 口袋控制台：会话/审批/提问/文件传输，局域网/Tailscale 直连，多服务器自动选优，聊天记录离线可看，Token 鉴权，数据只在设备间流动。 |
| ⭐ 8 | [xgone/dsh-remote](https://github.com/xgone/dsh-remote) | Remote access for DSH: account/password + MFA (TOTP) login gate, signed session cookies, role-based access, in-browser directory picker, Settings page. / DSH 远程访问：账号密码 + MFA (TOTP) 登录门、签名会话 Cookie、角色权限、浏览器内目录选择器、账户管理设置页。 |
| ⭐ 2 | [capriceyh/dsh-remote-conn](https://github.com/capriceyh/dsh-remote-conn) | DSH-remote-conn plugin: manage SSH machines, pick remote workspaces, operate remote files directly. / DSH 远程连接插件：管理 SSH 机器、选择远程工作区、直接操作远程文件。 |
| ⭐ 1 | [lnabc03/dsh-remote-gate](https://github.com/lnabc03/dsh-remote-gate) | Minimal DSH remote access gateway: mobile connects via frp/SSH reverse tunnel/Cloudflare tunnel (zero-server) or same LAN — safely operate your local DSH Web UI. / DSH 最小化远程网关：移动端经 frp/SSH 反向隧道/Cloudflare 隧道（零服务器）或同一局域网安全操控本地 DSH Web UI。 |
| ⭐ 0 | [mrgaoang/dsh-remote](https://github.com/mrgaoang/dsh-remote) | Remote control for DSH from any phone browser — secure gateway, mobile web client, full feature coverage (incl. privileged methods). / 手机浏览器远程控制 DSH：安全网关、移动端 Web 客户端，完整功能覆盖（含特权方法）。 |
| ⭐ 0 | [ruby1304/dsh-remote](https://github.com/ruby1304/dsh-remote) | Independent mobile PWA remote control for DeepSeek Harness. / 独立移动端 PWA 远程控制 DeepSeek Harness。 |
...
| ⭐ 1 | [lesliechowsh/dsh-memo](https://github.com/lesliechowsh/dsh-memo) | Memo — session memory search for DeepSeek Harness agents (memo_search / memo_remember / memo_stats on the official sessionQuery service). / DSH Agent 会话记忆搜索（基于官方 sessionQuery 服务的 memo_search / memo_remember / memo_stats）。 |
| ⭐ 0 | [00080000/dsh-project-memory](https://github.com/00080000/dsh-project-memory) | Read-time project memory plugin for DeepSeek Harness (dsh). / DSH 读取时项目记忆插件。 |
...
| ⭐ 1 | [Mason-1011/dsh-task-center](https://github.com/Mason-1011/dsh-task-center) | Task lifecycle plugin suite for DeepSeek Harness (dsh): agents claim work across sessions, wake up on schedule, quota-aware; idle/imported chats auto-mined into board candidates; full web kanban. / DSH 任务全生命周期插件族：跨会话认领、定时唤醒、额度感知、闲置会话自动抽取候选，配 Web 看板。 |
...
| ⭐ 52 | [kingOfSoySauce/dsh-skin-market](https://github.com/kingOfSoySauce/dsh-skin-market) | DeepSeek Harness skin market: 100+ DSH skins with scoring system and manual review, community submission portal, online browsing page and plugin for local skin management. / DSH 皮肤市场：已收录100+ DSH 皮肤，完善评分系统加人工审核，有便捷的社区收录入口，附在线页面与本地皮肤管理插件。 |
| ⭐ 4 | [qweqwe12382/dsh-client-ui-pet](https://github.com/qweqwe12382/dsh-client-ui-pet) | Q pet desktop companion overlay plugin for DeepSeek Harness web GUI (dsh bundle). / DSH Web GUI Q 宠物桌面伴侣悬浮插件（dsh bundle）。 |
...
| ⭐ 8 | [Js2Hou/dsh-mcp-manager](https://github.com/Js2Hou/dsh-mcp-manager) | DeepSeek Harness MCP server management plugin: visual interface management + on-demand hot injection, OAuth authentication support, environment variable configuration. / DSH MCP 服务器管理插件：可视化界面管理 + 按需热注入，OAuth 鉴权支持，环境变量配置。 |
...
| ⭐ 1 | [echo719/dsh-plugin-hub](https://github.com/echo719/dsh-plugin-hub) | dsh-plugin-hub.com — Find DeepSeek Harness plugins by problem. Audited before you install. / dsh-plugin-hub.com：按问题找 DSH 插件，安装前已审计。 |
| ⭐ 0 | [DshMarketPlace/dsh-plugin-validator](https://github.com/DshMarketPlace/dsh-plugin-validator) | Sandboxed install validation for DeepSeek Harness plugins — installs each one into a fresh profile in a throwaway container and reports what the harness recorded. / DSH 插件沙箱安装验证：在临时容器中新 profile 安装每个插件，报告 harness 记录内容。 |
...
| ⭐ 3 | [MicroHEROX/dsh-exa-mcp](https://github.com/MicroHEROX/dsh-exa-mcp) | Exa Search MCP for DeepSeek Harness: mounts the remote Exa MCP endpoint (https://mcp.exa.ai/mcp) through the in-box @deepseek-ai/dsh-mcp-client bridge. / DSH Exa Search MCP：通过内置 @deepseek-ai/dsh-mcp-client 桥接挂载远程 Exa MCP 端点。 |
| ⭐ 2 | [LongSir0419/dsh-mcp-manager](https://github.com/LongSir0419/dsh-mcp-manager) | DSH MCP manager: add/edit/enable/disable MCP server configs, bundled node resolution. / DSH MCP 管理器：添加/编辑/启用/停用 MCP 服务器配置，捆绑 node 解析。 |
| ⭐ 2 | [Yu-tao-Li/dsh-computer-use-win](https://github.com/Yu-tao-Li/dsh-computer-use-win) | Windows computer use for DeepSeek Harness (dsh): MCP stdio server + PowerShell UIA backend, 22 desktop tools. / DSH Windows 电脑操控：MCP stdio server + PowerShell UIA 后端，22 个桌面工具。 |
...
| ⭐ 3 | [xxxyz/DeepSeekHarness-MCP-Manager](https://github.com/xxxyz/DeepSeekHarness-MCP-Manager) | DSH durable MCP server manager plugin — Settings UI + HTTP API + mcp_manager_* model tools, cross-platform installers. / DSH 持久化 MCP 服务器管理插件：Settings UI + HTTP API + mcp_manager_* 模型工具，跨平台安装包。 |
| ⭐ 0 | [lwy521/dsh-remote-mcp-manager](https://github.com/lwy521/dsh-remote-mcp-manager) | DSH plugin: remote MCP server manager — configure and manage remote MCP Servers via DSH Web GUI settings panel. / DSH 插件：远程 MCP 服务器管理器，通过 DSH Web GUI 设置面板配置和管理远程 MCP Server。 |
| ⭐ 0 | [SeaOf0/dsh-mcp-studio](https://github.com/SeaOf0/dsh-mcp-studio) | DeepSeek Harness MCP server workbench: real-time mounting, connection status, tool preview, JSON one-click import. / DSH MCP 服务器工作台：实时挂载、真实连接状态、工具预览、JSON 一键导入。 |
| ⭐ 1 | [jinhongxun/dsh-mcphub](https://github.com/jinhongxun/dsh-mcphub) | MCP management panel for DSH: connection status, one-click pip/npx upgrades, pause/delete, add-server form, handshake probes. / DSH MCP 管理面板：连接状态、一键 pip/npx 升级、暂停/删除、添加服务器表单、握手探测。 |
| ⭐ 0 | [satan9394/dsh-mcp-builder](https://github.com/satan9394/dsh-mcp-builder) | DSH skill: MCP server construction — tool design and 4-stage process (inspired by Anthropic skills Apache-2.0). / DSH 技能：MCP 服务器构建，工具设计与四阶段流程（受 Anthropic skills Apache-2.0 启发）。 |
...
| ⭐ 0 | [nan1010082085/dsh-mcp-sync](https://github.com/nan1010082085/dsh-mcp-sync) | DSH plugin: sync MCP configs from Claude Code, Codex CLI, and Cursor Agent. / DSH 插件：从 Claude Code、Codex CLI 和 Cursor Agent 同步 MCP 配置。 |
| ⭐ 0 | [Nichtsv0v0/dsh-mcp-manager](https://github.com/Nichtsv0v0/dsh-mcp-manager) | MCP server manager for DeepSeek Harness: manage MCP servers from the Web settings page, stored in $DSH_HOME/mcp-servers.json, connect/disconnect at runtime. / DSH MCP 服务器管理器：Web 设置页管理 MCP 服务器，存储在 $DSH_HOME/mcp-servers.json，运行时连接/断开。 |
| ⭐ 0 | [hyzyn/dsh-mcp](https://github.com/hyzyn/dsh-mcp) | DSH Web GUI MCP server configuration plugin: manage ~/.dsh/cordis.patch.yml MCP section, support stdio/streamable-http, connection testing, enable/disable, hot reload as mcp__<server>__<tool> tools. / DSH Web GUI MCP 配置插件：管理 ~/.dsh/cordis.patch.yml MCP 区段，支持 stdio/streamable-http，连接测试，启用/禁用，热重载为 mcp__<server>__<tool> 工具。 |
| ⭐ 0 | [js2hou/dsh-mcp-manager](https://github.com/js2hou/dsh-mcp-manager) | MCP visual manager plugin for DeepSeek Harness: GUI to inspect installed/enabled MCP servers, add/remove, enable/disable and view real-time connection status. / DSH MCP 可视化管理插件：GUI 检查已安装/已启用的 MCP 服务器，添加/删除，启用/禁用，查看实时连接状态。 |
| ⭐ 0 | [dshplugin/dsh-plugin-hub](https://github.com/dshplugin/dsh-plugin-hub) | DSH built-in plugin marketplace: search and install 4000+ curated community plugins, daily updated, completely free. Built into Harness Settings → Plugin Center. / DSH 社区内置插件市场：搜索并安装 4000+ 人工精选社区插件，每日更新、完全免费。内置于 Harness「设置 → 插件中心」。 |
| ⭐ 0 | [webkong/dsh-plugin-manager](https://github.com/webkong/dsh-plugin-manager) | DSH plugin manager: install/uninstall/start/stop third-party plugins and GitHub direct links. / DSH 插件管理器：内置/三方插件安装、卸载、启动、停用与 GitHub 直跳。 |
| ⭐ 0 | [sehoon123/dsh-mcp-adapter](https://github.com/sehoon123/dsh-mcp-adapter) | Context-cheap, crash-resistant MCP access for the DeepSeek Harness: one lazy proxy tool instead of dumping every server's schema into context. Zero dependencies. / DSH 上下文廉价、抗崩溃的 MCP 访问：懒加载代理工具而非将 schema 全部塞入 context，零依赖。 |
| ⭐ 0 | [Daseanle/dsh-mcp-orchestrator](https://github.com/Daseanle/dsh-mcp-orchestrator) | MCP orchestration layer for DeepSeek Harness — multi-server routing, health monitoring, fallback, and tool aggregation. / DSH MCP 编排层：多服务器路由、健康监控、备用切换与工具聚合。 |
| ⭐ 0 | [Zenjibad/dsh-mcp-toggle](https://github.com/Zenjibad/dsh-mcp-toggle) | Enable/disable MCP servers directly from DSH Settings: a MCP Servers page that stops/starts each @deepseek-ai/dsh-mcp-client connection live and persists the change across restarts. / 直接在 DSH 设置中启用/停用 MCP 服务器：MCP Servers 页面可停止/启动每个 @deepseek-ai/dsh-mcp-client 连接并跨重启持久化。 |
...
| ⭐ 2 | [redfox-data/redfox-community-dsh](https://github.com/redfox-data/redfox-community-dsh) | The official bundle plugin package of DSH (DeepSeek Harness) from RedFoxHub: Over 100 social media data skills (Douyin / Xiaohongshu / Kuaishou / Bilibili / Official Accounts / Video Accounts / Weibo / YouTube / TikTok, etc.), installed in the native DSH skill format with just one click. / RedFoxHub 官方 DSH Bundle 插件包：100+ 社交媒体数据技能（抖音/小红书/快手/哔哩/公众号/视频号/微博/YouTube/TikTok 等），原生 DSH 技能格式一键安装。 |
| ⭐ 1 | [LongSir0419/dsh-skill-manager](https://github.com/LongSir0419/dsh-skill-manager) | DSH bundle: manage user-level skills from Settings — list all skills with enable/disable switches (disabled skills are excluded from model catalogs). / DSH Bundle：从设置页管理用户级技能——列出所有技能并支持启用/停用开关（停用的技能不进入模型目录）。 |
| ⭐ 1 | [YTyangtao666/dsh-skills-bridge](https://github.com/YTyangtao666/dsh-skills-bridge) | Bring your Claude Code skills into DeepSeek Harness — zero migration, one plugin. 一个插件把 Claude Code 技能桥接进 DeepSeek Harness。 |
| ⭐ 1 | [sun-gift/dsh-skills-settings_sl](https://github.com/sun-gift/dsh-skills-settings_sl) | DSH web plugin: manage installed Skills from the Settings UI (list/search/category filter, create, edit, disable, delete, use). / DSH Web 插件：从设置 UI 管理已安装的 Skills（列表/搜索/分类筛选、创建、编辑、停用、删除、使用）。 |
...
| ⭐ 0 | [xu-jin-cs/dsh-skills](https://github.com/xu-jin-cs/dsh-skills) | DSH ecosystem skill pack: parallel-dispatch parallel scheduling rules + archmap architecture mapping Agent — zero-LLM deterministic diff impact analysis, token-saving. / DSH 生态技能包：parallel-dispatch 并行调度规则 + archmap 架构测绘 Agent（零LLM确定性diff影响面，节约tokens）。 |
| ⭐ 0 | [satan9394/dsh-parallel-agent-ade](https://github.com/satan9394/dsh-parallel-agent-ade) | DSH skill: parallel agent development environment — worktree fan-out/merge, mobile companion, Design Mode, GitHub/Linear native, SSH remote, AI diff annotation (inspired by stablyai/orca 49k★). / DSH 技能：并行 agent 开发环境（worktree 扇出合并、移动伴侣、Design Mode、GitHub/Linear 原生、SSH 远程、AI diff 标注）（受 stablyai/orca 49k★ 启发）。 |
| ⭐ 0 | [satan9394/dsh-model-gateway](https://github.com/satan9394/dsh-model-gateway) | DSH skill: model routing gateway — 290+ providers single endpoint, quota-aware auto-fallback, 19 routing strategies, RTK+Caveman compression saves 15-95%, honest free-tier budget (inspired by diegosouzapw/OmniRoute 51k★). / DSH 技能：模型路由网关（290+ 提供商一端点、配额感知自动回退、19 种路由策略、RTK+Caveman 压缩省 15-95%、免费层诚实预算）（受 diegosouzapw/OmniRoute 51k★ 启发）。 |
| ⭐ 0 | [satan9394/dsh-autonomous-research](https://github.com/satan9394/dsh-autonomous-research) | DSH skill: autonomous research agent — modify code → fixed 5-min training → val_bpb judge优劣 → keep/discard loop, single-file modification, program.md ultra-lightweight skill, ~100 experiments overnight (inspired by karpathy/autoresearch 94k★). / DSH 技能：自主研究 agent（改代码→固定 5 分钟训练→val_bpb 判优劣→保留/丢弃循环、单文件修改、program.md 超轻量 skill、一晚约 100 实验）（受 karpathy/autoresearch 94k★ 启发）。 |
| ⭐ 0 | [satan9394/dsh-self-improving-agent](https://github.com/satan9394/dsh-self-improving-agent) | DSH skill: self-improving AI agent — closed-loop learning回路, skill self-creation & self-improvement, cross-session recall, Honcho user modeling, cross-platform gateway, cron scheduling, serverless sleep (inspired by NousResearch/hermes-agent 154k★). / DSH 技能：自改进 AI agent（闭环学习回路、技能自创建自改进、跨会话回忆、Honcho 用户建模、跨平台网关、cron 调度、serverless 休眠）（受 NousResearch/hermes-agent 154k★ 启发）。 |
| ⭐ 0 | [satan9394/dsh-knowledge-brain](https://github.com/satan9394/dsh-knowledge-brain) | DSH skill: knowledge brain layer — synthesized answers with citations, self-connected knowledge graph zero LLM calls, gap analysis, 24/7 dream cycle, company brain login isolation (inspired by garrytan/gbrain 29k★). / DSH 技能：知识大脑层（合成答案带引用、自连线知识图谱零 LLM 调用、差距分析、24/7 梦境周期、公司大脑登录隔离）（受 garrytan/gbrain 29k★ 启发）。 |
...
| ⭐ 0 | [JohnXu22786/review-gate](https://github.com/JohnXu22786/review-gate) | DeepSeek Harness (dsh) bundle that turns code review into a hard gate: deterministic severity rules, LLM-assisted findings, team approval quorum and a compliance audit trail. / DSH Bundle：将代码审查变为硬性门槛：确定性严重性规则、LLM 辅助发现、团队批准法定人数与合规审计追踪。 |
| ⭐ 1 | [1919-doomer/DeepseekHarness-CLI](https://github.com/1919-doomer/DeepseekHarness-CLI) | A terminal-native CLI/TUI frontend for DeepSeek Harness, inspired by Codex CLI and Claude Code. / DSH 终端原生 CLI/TUI 前端，灵感来自 Codex CLI 和 Claude Code。 |
| ⭐ 0 | [haoku123/dsh-budget-guard](https://github.com/haoku123/dsh-budget-guard) | DeepSeek Harness plugin: LLM cost metering with budget enforcement (deny/warn) and peak-valley pricing. / DSH 插件：成本计量+超预算硬拦截，支持 DeepSeek 峰谷定价。 |
| ⭐ 0 | [haoku123/dsh-blast-radius](https://github.com/haoku123/dsh-blast-radius) | DeepSeek Harness plugin: semantic blast radius for agent code edits — who calls what the agent just changed, and which callers have no test. / DSH 插件：Agent 代码编辑语义爆炸半径分析。 |
| ⭐ 0 | [amphilagus/dsh-dark-room](https://github.com/amphilagus/dsh-dark-room) | Out-of-tree DSH bundle: black-room policy that keeps the agent inside the workspace and off the internet. / DSH Bundle：黑房策略，保持 Agent 在工作区内不上互联网。 |
| ⭐ 1 | [amphilagus/dsh-literature](https://github.com/amphilagus/dsh-literature) | Out-of-tree DSH bundle for scientific literature search and tracking, with a 文献跟踪助理 agent preset. / DSH Bundle：科学文献搜索与追踪，附文献跟踪助理 Agent 预设。 |
| ⭐ 1 | [Sivan757/dsh-agent-plugins-market](https://github.com/Sivan757/dsh-agent-plugins-market) | DeepSeek Harness (DSH) plugin marketplace: install and inject Claude Code / Codex / Cursor agent plugins — skills, MCP servers, hooks and slash commands — from git marketplace repos, with a Web GUI market page. / DSH 插件市场：从 Git 市场仓库安装并注入 Claude Code/Codex/Cursor Agent 插件，含 Web GUI 市场页面。 |
